...
public class PacketGate extends ListenerOwner {
  ...
  public void registerListener(
      PacketListener packetListener, ListenerPriority priority, Class... packetClasses) {
    ...
    if (packetClasses.length == 0) {
      for (EnumConnectionState state : EnumConnectionState.values()) {
        try {
          ...
          directionMaps.forEach(
              (enumPacketDirection, integerClassBiMap) -> {
                integerClassBiMap.forEach(
                    (id, clazz) -> {
                      classes.add(clazz);
                    });
              });
      ...
      for (Class clazz : packetClasses) {
        Preconditions.checkArgument(
            Packet.class.isAssignableFrom(clazz),
