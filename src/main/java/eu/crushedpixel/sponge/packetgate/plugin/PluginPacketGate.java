...
public class PluginPacketGate {
  ...
  public void init(GameInitializationEvent event) {
    ...
    try {
      // Forge updated the accessibility of the endpoints to private, make sure it is accessible
      // first
      Arrays.stream(networkSystem.getClass().getDeclaredFields())
      ...
      endpoints.forEach(
          channelFuture -> {
            channelFuture
                .channel()
                .pipeline()
                .addFirst(new CustomChannelInitializer(logger, packetGate));
